...
static const struct snd_soc_dapm_widget es8326_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("LHPMIX", SND_SOC_NOPM, 0, 0, NULL, 0),
	SND_SOC_DAPM_PGA("RHPMIX", SND_SOC_NOPM, 0, 0, NULL, 0),
...
static int es8326_resume(struct snd_soc_component *component)
{
	...
	regmap_write(es8326->regmap, ES8326_CLK_INV_02, 0x08);
	regmap_write(es8326->regmap, ES8326_CLK_DIV_CPC_0C, 0x1F);
	regmap_write(es8326->regmap, ES8326_CLK_VMIDS1_10, 0xC4);
	regmap_write(es8326->regmap, ES8326_CLK_VMIDS2_11, 0x80);
	regmap_write(es8326->regmap, ES8326_CLK_CAL_TIME_12, 0x04);
	...
	regmap_write(es8326->regmap, 0x24, 0xaf);
	regmap_write(es8326->regmap, 0x23, 0x1a);
	regmap_write(es8326->regmap, 0x29, 0x55);
	regmap_write(es8326->regmap, 0x2c, 0xe1);
	regmap_write(es8326->regmap, 0x2d, 0xe1);
	regmap_write(es8326->regmap, 0x51, 0x1a);
	regmap_write(es8326->regmap, 0x1b, 0x7c);
#endif
        es8326->chip_state = 1;
				...
				struct snd_soc_dai *dai)
{
	...
	printk("es8326_pcm_hw_params %d", substream->stream);
	if(substream->stream == 0){/* playback */
	if(es8326->chip_state == 0)
	        es8326_resume(codec);
	regmap_write(es8326->regmap,  0x4e, 0xe1);	
	queue_delayed_work(system_wq, &es8326->pcm_pop_work, msecs_to_jiffies(25));
	}
	//coeff = get_coeff(es8326->sysclk, params_rate(params));
        coeff = get_coeff(4800000, params_rate(params));
	/* bit size */
...
static void es8326_pcm_shutdown(struct snd_pcm_substream *substream,
			      struct snd_soc_dai *dai)
{
	struct snd_soc_component* component = dai->component;
	struct es8326_priv *es8326 = snd_soc_component_get_drvdata(component);	

        printk("es8326_pcm_shutdown %d", substream->stream);

	if(substream->stream==0){/* playback */
	cancel_delayed_work(&es8326->pcm_pop_work); 
	regmap_write(es8326->regmap,  0x27, 0x00);
	regmap_write(es8326->regmap,  0x4e, 0x20);

	//regmap_write(es8326->regmap,  0x00, 0x8f);
	}
}
...
{
	struct es8326_priv *es8326 =
		container_of(work, struct es8326_priv, pcm_pop_work.work);
	struct snd_soc_component *comp = es8326->component;	
	unsigned int muteflag;
	unsigned int flag_27;
	
#if 1
	regmap_write(es8326->regmap,  0x00, 0x80);
	regmap_read(es8326->regmap, 0x50, &flag_27);
	if(flag_27!=0)
		es8326->dac_vol = flag_27;
	muteflag = snd_soc_component_read(comp, 0xfc);
	if(muteflag & 0x40){
	if((muteflag & 0x01)){	        
		...
		if(chip_state==16){			
			regmap_write(es8326->regmap,  0x27, 0x00);			
			regmap_write(es8326->regmap, 0x25, 0x11);
			//regmap_write(es8326->regmap, 0x17, 0xF8);
			regmap_write(es8326->regmap, 0x24, 0x03);
			regmap_write(es8326->regmap,  0x50, 0x83);
			printk("es8326 pcm 27 00");
			chip_state=1;
		}
	}else if(chip_state > 0){
		regmap_write(es8326->regmap,  0x50, es8326->dac_vol);
		regmap_write(es8326->regmap, 0x24, 0x00);
		regmap_write(es8326->regmap, 0x17, 0x40);		
		regmap_write(es8326->regmap,  0x4d, 0x00);		
		regmap_write(es8326->regmap,  0x27, 0x77);
		regmap_write(es8326->regmap, 0x25, 0xAA);		
		chip_state=0;
		printk("es8326 pcm normal flag %#04x",muteflag);
	}
	}
	queue_delayed_work(system_wq, &es8326->pcm_pop_work, msecs_to_jiffies(25));
#endif
...
static void __exit pcm_pop_work_exit(struct es8326_priv *es8326)  
{  
    int ret;  
    ret = cancel_delayed_work(&es8326->pcm_pop_work);   
} 
...
static int es8326_probe(struct snd_soc_component *component)
{
	...
	INIT_DELAYED_WORK(&es8326->pcm_pop_work,
				pcm_pop_work_events);
